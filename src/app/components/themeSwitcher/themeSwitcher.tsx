...
const ThemeSwicther = ({
  ...
  const handleFilterChange = async event => {
    ...
    if (value) {
      results = themes.filter(
        theme => theme.toUpperCase().indexOf(value.toUpperCase()) >= 0
      );
  ...
  const handleResetStyle = () => {
    ...
    setPopupMessage(
      `Successfully reset current theme and applied the default theme`
    );
  };
  if (data && data.length === 0) {
    return (
      <div style={{ display: "flex" }} className="noThemesAvaile">
        ...
        >
          <h3 style={{ display: "inline", marginLeft: "3px" }}>
            No Themes Found
          </h3>
            ...
            <div className={`searchField ${filterInputClassName}`}>
              ...
              <a className="search-btn">
                <svg
                  ...
                  onClick={() => {
                    handleFilterToggle(
                      filterInputClassName === "" ? show : hide
                    );
                  }}
                >
                  <path
