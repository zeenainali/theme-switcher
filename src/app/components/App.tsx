...
const App = ({}) => {
  ...
  const [applyThemePopupHidden, setApplyThemePopupHidden] = React.useState(
    true
  );
  ...
  React.useEffect(() => {
    (async function() {
      await parent.postMessage(
        { pluginMessage: { type: "getCurrentTheme" } },
        "*"
      );
      getCreatedTheme();
    })();
  ...
  const getCreatedTheme = () => {
    window.onmessage = event => {
      ...
      if (type === "getTheme") {
        ...
        setData(
          collectLocalColors && collectLocalColors.length > 0
            ? collectLocalColors
            : []
        );
      }
