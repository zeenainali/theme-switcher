  ...
  if (msg.type === "getCurrentTheme") {
    (async function() {
      try {
        ...
        const isOnBoardingDone = await figma.clientStorage.getAsync(
          "isOnBoardingDone"
        );
...
function findMatchInSelectedTheme(styleKey, data) {
  ...
  const currentStyle = data.find(
    (style: { key: any }) => style.key === styleKey
  );
  ...
  if (currentStyle) {
    ...
    const matchedStyle = data.find(
      (style: { name: any; theme: string }) =>
        style.name === name && style.theme === getSelectedTheme
    );
    if (matchedStyle) {
...
function getLocalPaintsArray(types: string) {
  ...
  (async function() {
    try {
      const themeData = await figma.clientStorage.getAsync(
        "figma-local-variables"
      );
      const isOnBoardingDone = await figma.clientStorage.getAsync(
        "isOnBoardingDone"
...
function applyTheme(applyTo: string, themeName: any) {
  (async function storeData() {
    try {
      const ThemeObj = await figma.clientStorage.getAsync(
        "figma-local-variables"
      );
      ...
      if (nodes) {
        ...
        const colorStyles = [
          ...new Set(
            data.map(
              style => style.theme === themeName && style.type === "PAINT"
            )
          )
        ];
        if (colorStyles) {
